...
function componentToHTML(component: ComponentNode): string {
  ...
  switch (component.type) {
    ...
    case 'section': {
      ...
      if (hasColumns) {
        // MSO Conditional Table (Ghost Table) start
        const msoStart = `
          <!--[if mso | IE]>
        ...
        // MSO Conditional Table (Ghost Table) end
        const msoEnd = `
          <!--[if mso | IE]>
          </tr>
          </table>
        ...
        // Render columns with MSO cell wrappers
        const columnsHTML = component.children
          ? component.children.map(child => {
            if (child.type !== 'column') return componentToHTML(child);

            // Wrap each column in MSO conditional cell
            return `
                <!--[if mso | IE]>
                <td width="${child.props.width || '50%'}" valign="top" style="vertical-align: top; width: ${child.props.width || '50%'};">
                <![endif]-->
                ${componentToHTML(child)}
                <!--[if mso | IE]>
                </td>
                <![endif]-->
              `;
        ...
        return `
        ${msoStart}
        <table border="0" cellpadding="0" cellspacing="0" role="presentation" width="100%"${styleAttr} style="${inlineStyles}; border-collapse: collapse; mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
          <tr>
...
<head>
  ...
  <style type="text/css">
    ...
    @media only screen and (max-width: 600px) {
      ...
      td[class="column"] { display: block !important; width: 100% !important; }
